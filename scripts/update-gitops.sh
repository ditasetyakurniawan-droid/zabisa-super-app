...
PROJECT="${PROJECT:-devops-apps/zabisa}"; PROJECT="${PROJECT#/}"; PROJECT="${PROJECT%/}"
...
for name in "${ZABISA_IMAGE_NAMES[@]}"; do
  old="harbor-dt.co.id/devops-apps/zabisa/${name}:REPLACE_SHA"
...
actual="$(grep -Rh --include='*.yaml' --include='*.yml' -E '^[[:space:]]*image:[[:space:]]+harbor-dt\.co\.id/devops-apps/zabisa/' "$DEST/manifests" | wc -l | tr -d ' ')"
